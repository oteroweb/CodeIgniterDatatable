...
CREATE TABLE IF NOT EXISTS `clientes` (
  ...
  `CedulaIdentidadl` varchar(45) NOT NULL,
...
ALTER TABLE `detalleventas`
  ADD CONSTRAINT `detalleventas_ventas` FOREIGN KEY (`iddetalleventas`) REFERENCES `ventas` (`idventas`) ON DELETE NO ACTION ON UPDATE NO ACTION;
...
ALTER TABLE `productos`
  ADD CONSTRAINT `productos_detalleventa` FOREIGN KEY (`idproductos`) REFERENCES `detalleventas` (`productos_idproductos`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `ventas`
  ...
  ADD CONSTRAINT `ventas_ibfk_1` FOREIGN KEY (`terminoventas_idterminoventas`) REFERENCES `terminoventas` (`idterminoventas`);
COMMIT;
